...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, offline, meal_data, setTempBasal) {
    ...
    var mealAssistPct = 0;
    //var wtfAssistPct = 0;
    // if BG is high (more than DIA/2 hours of basal above max_bg, i.e. above about 180mg/dL) and rising, wtf-assist
    var high = profile.max_bg + ( basal * (profile.dia/2) * profile.sens );
    if ( bg > high && minDelta > Math.max(0,bgi) ) {
        wtfAssist=1;
    }
    // minDelta is > 10 and devation is > 40 wtf-assist and meal-assist
    var wtfDeviation=40;
    var wtfDelta=10;
    ...
    if (
        // if minDelta is >3 and >BGI, or >=0 if BGI is positive, and...
        (minDelta > Math.max(3, bgi) || ( minDelta >= 0 && bgi > 0 )) && 
        // there are uncovered carbs, meal-assist
        (meal_data.carbs > 0 && remainingMealBolus > 0)
      ) {
        mealAssist=1;
    }
    ...
    if (wtfAssist > 0) {
        ...
        mealAssistPct = Math.round(Math.max(mealAssist,wtfAssist)*100);
        //wtfAssistPct = Math.round(wtfAssist*100);
        rT.mealAssist = "On: "+mealAssistPct+"%, Carbs: " + meal_data.carbs + " Boluses: " + meal_data.boluses + " Target: " + target_bg + " Deviation: " + deviation + " BGI: " + bgi;
    } else {
    ...
    if (bg < threshold) { // low glucose suspend mode: BG is < ~80
        ...
        return setTempBasal(basal, 30, profile, rT, offline);
        /*
    ...
    //if (remainingMealBolus > 0 && snoozeBG + remainingMealBolus*profile.sens > min_bg && minDelta > Math.max(0,expectedDelta)) {
    if (remainingMealBolus > 0 && snoozeBG + remainingMealBolus*profile.sens > min_bg && minDelta > expectedDelta) {
    ...
    if (eventualBG < min_bg) { // if eventual BG is below target:
        ...
        if (eventualBG < min_bg) {
            ...
            if (snoozeBG > min_bg) { // if adding back in the bolus contribution BG would be above min
                ...
                /*
                if (glucose_status.delta < 0 && currenttemp.duration > 0 && currenttemp.rate > basal) {
                    rT.reason += tick + ", and temp " + currenttemp.rate + " > basal " + basal;
                    return setTempBasal(0, 0, profile, rT, offline); // cancel temp
                } else if (glucose_status.delta > 0 && currenttemp.duration > 0 && currenttemp.rate < basal) {
                    rT.reason += tick + ", and temp " + currenttemp.rate + " < basal " + basal;
                    return setTempBasal(0, 0, profile, rT, offline); // cancel temp
                }
                */
    ...
    if (minDelta < expectedDelta) {
        if (glucose_status.delta < glucose_status.avgdelta) {
            ...
            rT.reason += "Eventual BG " + eventualBG + ">" + min_bg + " but Avg. Delta " + glucose_status.avgdelta.toFixed(2) + " < Exp. Delta " + expectedDelta;
        }
        //if (currenttemp.duration > 0) { // if there is currently any temp basal running
            //rT.reason +== "; cancel";
            //return setTempBasal(0, 0, profile, rT, offline); // cancel temp
    ...
    if (eventualBG < profile.max_bg) {
        ...
        //if (offline == 'Offline') {
            ...
            //if ((!currenttemp.duration || (currenttemp.rate == basal)) && !rT.duration) {
                rT.reason += "; setting current basal of " + basal + " as temp";
                return setTempBasal(basal, 30, profile, rT, offline);
            //}
        //}
    ...
    if (basaliob > max_iob) {
        ...
        rate = Math.round( rate * 1000 ) / 1000;
            ...
            var maxSafeBasal = Math.min(profile.max_basal, 4 * profile.max_daily_basal, 5 * basal);
